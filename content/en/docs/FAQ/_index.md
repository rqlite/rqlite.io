...
No, you must only change the database using the HTTP API. The moment you directly modify the SQLite file, including any changes to its journaling mode, under any node, the behavior of rqlite is undefined. In otherwords, you run the risk of breaking your cluster and losing data.

## Can I read the SQLite file directly?
Yes, you can read the SQLite file directly, some end-users do this in production, but it has not been deeply tested. But you must ensure your accesses are **read-only**. If you make **any direct change** to the SQLite database, or **modify the SQLite files in any way**, you risk breaking rqlite and experiencing data loss. If you do decide to read the SQLite file directly, it is strongly recommended that your readers open explicitly [read-only connections](https://www.sqlite.org/c3ref/open.html), and that you also configure file permissions such that your readers are prevented by the Operating System from making any changes to the SQLite files.
