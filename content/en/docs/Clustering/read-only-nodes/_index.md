...
A read request to a read-only node must use a [read-consistency level](/docs/api/read-consistency/) of `none` or `auto`. If any other level is specified — or if no level is set explicitly — the read-only node will forward the request to the Leader, negating the benefits of using a read-only node.
