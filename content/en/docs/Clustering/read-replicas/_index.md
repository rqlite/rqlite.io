...
The name _read replica_ describes the node's role in replication and consensus -- it does **not** mean the node rejects write requests. If a client sends a write request to a read replica, the replica transparently forwards that request to the Leader, waits for the Leader's response, and returns the response to the client. From your application's point of view, a read replica accepts exactly the same requests as any other rqlite node. What makes a read replica different is the read path: it does this without .
...
- **Large-scale fan-out**: The practical limit for voting nodes in a cluster is about 9-11 nodes, but there is no such limit on read replicas. If you need to broadcast changes to tens nodes, read replicas let you do it: every replica receives every change committed by the cluster.
