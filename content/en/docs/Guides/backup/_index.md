...
> Make sure there is no other write traffic being sent to your rqlite system while you are restoring from a backup. Also a _Restore_ operation should be monitored, and if it fails it is best to start again with a new cluster before re-attempting the operation.
