...
_Booting_ is a specialized process that enables rapid initialization of a node from a SQLite database image. This method is designed for high-efficiency data loading, particularly suited for disaster recovery or initializing a large database quickly. Unlike the other restore processes, _Booting_  bypasses the Raft consensus mechanism, significantly reducing the load time. The only limiting factor is how fast your disks are.
