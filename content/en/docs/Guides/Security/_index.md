...
### Query security
All SQL statements sent to the [`/db/query` endpoint](/docs/api/api/) are executed using a read-only SQLite connection, ensuring that no Query request can modify the underlying database. Depending on your use case, limiting access to this endpoint only can enhance security.
